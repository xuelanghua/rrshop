...
<ul class="menu-head-top">
    <li <?php  if($_W['action']=='order') { ?> class="active" <?php  } ?>><a href="<?php  echo merchUrl('order')?>">订单概述 <i class="fa fa-caret-right"></i></a></li>
</ul>

<?php if(mcv('order.list.status1|order.list.status2|order.list.status0|order.list.status3|order.list.status_1|order.list.main')) { ?>
...
<ul class='order-list'>
    ...
    <?php  } ?>
     ...
     <?php if(mcv('order.list.status_1')) { ?>
...
<?php  } ?>
  <?php if(mcv('order.list.status4|order.list.status5')) { ?>
