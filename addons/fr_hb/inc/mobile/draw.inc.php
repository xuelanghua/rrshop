    ...
    if (empty($project) || $project['start_time'] > TIMESTAMP || $project['end_time'] < TIMESTAMP) {
        ...
        if (!empty($draws)) {
            ...
            if ($act == 'draw' && $isDraw) {
                ...
                if (sendDrawReward($openid, $draw, $project, $isFollow)){
                    $money = $draw['prize'] / 100;
                    $msg = '';
                    //发信息通知用户
                    if ($draw['type'] == 1) {
                        $url = murl('mc/bond/credits', array('credittype' => 'credit1'), false, true);
                        $msg = "恭喜您,抽中奖品“{$draw['name']}”获得积分奖励：{$money}<a href='{$url}'>查看</a>";
                    }
                    elseif($draw['type'] == 2) {
                        $url = murl('mc/bond/credits', array('credittype' => 'credit2'), false, true);
                        $msg = "恭喜您,抽中奖品“{$draw['name']}”获得余额奖励：{$money}<a href='{$url}'>查看</a>";
                    }
                    elseif($draw['type'] == 3){
                        $msg = "恭喜您,抽中奖品“{$draw['name']}”获得现金红包：{$money}元，请到您的微信钱包查看。";
                    }
                    elseif($draw['type'] == 4){
                        $msg = "恭喜您,抽中奖品“{$draw['name']}”获得卡券，请到您的微信卡包查看。";
                    }
                ...
                $log_id = pdo_insertid();
                $isz = true;
                if ($draw['prize'] > 0 && $draw['type'] != 4) {
                    $info = '恭喜您抽中了'.$draw['name'];
                }elseif ($draw['type'] == 4 && !empty($draw['card_id'])) {
                    $info = '恭喜您抽中了'.$draw['name'];
                }else {
                    $isz = false;
                    $info = '谢谢参于';
