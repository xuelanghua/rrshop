...
if (!empty($packet)) {
    ...
    $bg = $project['index_bg'] ? $project['index_bg'] : $bg;
    $tips_bg = $project['tips_bg'] ? $project['tips_bg'] : $tips_bg;
    $share_bg = $project['share_bg'] ? $project['share_bg'] : $share_bg;
}else {
    $isReceive = 0;
    $tips = "红包不存在或已失效";
}
...
if ($isReceive == 999 && !empty($openid)) {// 
    ...
    if ($frs ==  md5($_SESSION['fr_open_packet_session'] . $token)) {
        ...
        if (openPacket($openid, $packet, $project)) {
